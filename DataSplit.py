...
def set_new_dataframe(dataframe, case_index):
    ...
    new_dataframe = new_dataframe.append(dataframe.loc[case_index, :], ignore_index=False)
...
class DataSplit(object):
    ...
    def run(self, df, repeat_times=1, store_folder='', random_state=None, feature_select=None):
        ...
        if store_folder != '':
            ...
            output_train_df.to_csv(os.path.join(store_folder, 'train_numeric_feature.csv'), index=True,
                                   encoding="utf_8_sig")
            output_test_df.to_csv(os.path.join(store_folder, 'test_numeric_feature.csv'), index=True,
                                  encoding="utf_8_sig")
...
if __name__ == '__main__':
    data_path = r'C:\Users\HJ Wang\Desktop\ML\My_work\221017EC_MTL\radiomics\P53 NEW\combine\total.csv'
    data = pd.read_csv(data_path, index_col=0)
    data = data.replace(np.inf, np.nan)
    data = data.dropna(axis=1, how='any')

    data_split = DataSplit()
    train, test, _ = data_split.run(data, random_state=280,
